# Aether Edge – GRC Portfolio
### Managed by Kayasah Thompson|IT Risk Management Professional 

Welcome! I am a Cybersecurity Management and Policy student at the University of Maryland Global Campus. This portfolio showcases my ability to bridge the gap between technical security controls and business risk management through standardized GRC deliverables.

---

## Core Credentials
* **CompTIA Security+ Certified** 
* **CompTIA Network+ Certified**
* **Interim Security Clearance**
* **B.S. Cybersecurity Management & Policy** (Recent Graduate)

---

## Featured GRC Projects

### 1. [Quantitative Risk Register: AI Fintech Startup](./Project-Risk-Register/)
* **The Scenario:** Developed a risk management plan for a high-growth fintech firm utilizing AI.
* **Key Deliverables:** Asset valuation (AV), calculation of SLE/ALE for flood and data breach scenarios, and a prioritized risk heat map.
* **Frameworks:** NIST SP 800-30.

### 2. [Vendor Risk Assessment: Cloud Integration](./Project-Vendor-Risk/)
* **The Scenario:** Conducted a security posture review for a potential third-party SaaS vendor.
* **Key Deliverables:** Completed SIG questionnaire, risk scoring, and executive "Go/No-Go" recommendation.
* **Skills:** Third-party risk management (TPRM), control validation.

### 3. [NIST CSF 2.0 Control Mapping Exercise](./Project-Control-Mapping/)
* **The Scenario:** Mapped existing organizational security policies to the NIST Cybersecurity Framework 2.0.
* **Key Deliverables:** Gap analysis report and a remediation roadmap for non-compliant controls.

---

## Skills & Tools
* **Frameworks:** NIST CSF, NIST SP 800-53, ISO 27001, GDPR.
* **Risk Analysis:** Quantitative (ALE/SLE/ARO) and Qualitative assessment.
* **Documentation:** Policy drafting, Standard Operating Procedures (SOPs), Executive Summaries.
* **Tech Stack:** GitHub (Version Control), Excel (Data Modeling), GRC Tool Simulations.

---

## Let's Connect
I am actively seeking GRC Analyst and Information Security Officer (ISSO) roles.

📧 [kayasahthompson@gmail.com](mailto:kayasahthompson@gmail.com)
🔗 [LinkedIn Profile](https://www.linkedin.com/in/kayasah-thompson-706275186/)
